...
| MCP 会话隔离 | 每次 API 请求使用新的网页对话，避免客户端完整历史与网页历史重复叠加；自动过滤上一轮工具协议回显 |
...
| 对话防卡死 | 流式连接立即响应并定时保活；登录失效、页面错误或发送失败会快速返回明确错误，不再长时间空等或重复发送 |
| 消息编码兼容 | 自动识别 UTF-8、UTF-16、GB18030，并用 JSON 有效性校验，避免中文被转换成 `����` |
| 乱码历史自愈 | 自动跳过旧会话中已经损坏的消息；仅当前用户输入损坏时提示新建会话 |
